...
for breedName in breeds:
    ...
    # shape = {}
    DATA_DIR = os.getcwd() + "/images/"+breedName+"/"
    # imageList=os.listdir(DATA_DIR)
    imageList=[f for f in os.listdir(DATA_DIR) if f[-4:] == ".jpg"]
    tempList=[]
    trainfull=[]
    for path in imageList:
        ...
        if currShape == (400, 300, 3):
            ...
            trainfull.append(path)
    train, test= train_test_split(tempList, random_state=SEED, test_size=testSize)
    train, val = train_test_split(train, random_state=SEED, test_size=valSize)
    train=train+trainfull
    # train
    currentFolder="Train"
    count = 1
    for path in train:
        ...
        # if currShape == (400, 300, 3):
        # print(currShape)
        # print(np.max(currShape))
        desired_size = np.max(currShape)
        ...
        color = [0, 0, 0]
        finalImage = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                    value=color)
        # finalImage = cv2.resize(new_im, (finalSize, finalSize))
        filePath=os.getcwd() + "/finalImages2/"+currentFolder+"/"+breedName+"/"+breedName+"/"+str(count)+".jpg"
    ...
    for path in val:
        ...
        if currShape == (400, 300, 3):
            ...
            finalImage = cv2.copyMakeBorder(im, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                            value=color)
            # finalImage = cv2.resize(new_im, (finalSize, finalSize))
            filePath = os.getcwd() + "/finalImages2/" + currentFolder + "/" + breedName + "/" + breedName + "/" + str(
                count) + ".jpg"
            cv2.imwrite(filePath, finalImage)
            count += 1
...
#
#
# np.save("Data/GAN_x_trainSet1.npy", x_train);
# np.save("Data/GAN_y_trainSet1.npy", y_train);
# np.save("Data/GAN_x_val.npy", x_val);
# np.save("Data/GAN_y_val.npy", y_val);
# np.save("Data/GAN_x_test.npy", x_test);
# np.save("Data/GAN_y_test.npy", y_test);
